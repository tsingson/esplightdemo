...
const int INPUT_PIN = 2; // D2 接口
const int OUTPUT_PIN = 13; // D13 接口

std::atomic<int> old_state(0);

// 中断服务函数
void IRAM_ATTR handlePinChange()
{
    int state = digitalRead(INPUT_PIN);
    int targetMode = old_state.load();
    ...
    if (state == HIGH)
    {
        ...
        if (state != targetMode)
        {
            ...
            ets_printf("State changed: INPUT LOW, OUTPUT HIGH\n");
            // old_state = state;//
        }
    }
    else
    {
        ...
        {
            old_state.store(state);
            ets_printf("State changed: INPUT LOW, OUTPUT LOW\n");
            // old_state = state;
        }
    }
}
...
void setup()
{
    ...
    digitalWrite(OUTPUT_PIN, (initialState == HIGH) ? LOW : HIGH);

    attachInterrupt(digitalPinToInterrupt(INPUT_PIN), handlePinChange, CHANGE);

    Serial.println("System Initialized. Monitoring D2 state changes...");
